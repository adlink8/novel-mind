...
export function QualityStatusBadge({ status }: { status: string }) {
  ...
  return (
    <Badge data-testid={`quality-status-${status}`} className={cn("text-xs", TONE_CLASS[tone])}>
      {label}
    </Badge>
  );
}
...
export function DeprecationBanner({ meta }: { meta: DeprecationMeta }) {
  ...
  return (
    <div data-testid="deprecation-banner" className="rounded-xl border border-amber-300/60 bg-amber-50 px-4 py-3 text-xs text-amber-900">
    ...
    <div className="space-y-3" data-testid="quality-jobs-panel">
          ...
          <CardContent className="flex items-start justify-between gap-3 p-4">
            <div className="min-w-0 flex-1">
              <div className="mb-1 flex flex-wrap items-center gap-2">
                ...
                {!job.quality_comparable && <Badge variant="outline" className="text-xs">metrics=null</Badge>}
              </div>
              {job.quality_comparable && job.metrics && (
                <p className="text-xs text-muted-foreground">
                  faithfulness={(job.metrics.answer_faithfulness ?? 0).toFixed(2)} · recall@5={(job.metrics.context_recall_at_5 ?? 0).toFixed(2)}
                </p>
              )}
              {job.error && <p className="mt-1 text-xs text-red-600">{String(job.error)}</p>}
            </div>
            <div className="flex shrink-0 gap-2">
              <Button size="sm" variant="outline" onClick={() => onSelect?.(job.job_id)}>查看报告</Button>
              <Button size="sm" variant="ghost" onClick={() => onResume?.(job.job_id)}>恢复</Button>
            </div>
          </CardContent>
        </Card>
      ))}
      {selected && (
        <Card className="paper-surface rounded-2xl border-primary/30" data-testid="quality-job-report">
          <CardHeader><CardTitle className="text-base">质量报告: {selected.job_id}</CardTitle></CardHeader>
          <CardContent className="space-y-2 text-sm">
            <QualityStatusBadge status={String(selected.status)} />
            <p>quality_comparable: {selected.quality_comparable ? "true" : "false"}</p>
            <pre className="overflow-x-auto rounded-lg bg-muted/50 p-3 text-xs">
              {JSON.stringify(selected.quality_comparable ? selected.metrics : null, null, 2)}
            </pre>
          </CardContent>
        </Card>
      )}
