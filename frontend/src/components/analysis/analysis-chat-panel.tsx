 ...
 *
 * 锚点 = 结构：当前结构选中范围（章节区间）显示在上下文提示里。
 * 诚实降级：现有消息 API 仅支持单章 chapter_id 锚点（无区间锚点），
 * 发送时锚定到「区间末章 ∩ 剧透边界」，并在 UI 明示；完整区间锚点待 25.1-01。
 */
...
export function AnalysisChatPanel({
  ...
  /**
   * 单章上下文锚点（现有 API 能力）：区间末章 ∩ 剧透边界。
   * 无结构选中时：非全书=进度章；全书=末章。
   */
  const anchorChapterNumber = useMemo(() => {
    if (selection) {
      if (fullBook) return selection.chapterEnd;
      if (cutoffChapterNumber == null) return null;
      return Math.min(selection.chapterEnd, cutoffChapterNumber);
    }
    if (fullBook) return lastChapterNumber;
    return cutoffChapterNumber;
  }, [selection, fullBook, cutoffChapterNumber, lastChapterNumber]);

  const anchorChapter = useMemo(
    () =>
      anchorChapterNumber == null
        ? null
        : chapters.find((c) => c.chapter_number === anchorChapterNumber) ??
          null,
    [chapters, anchorChapterNumber]
  );

  const anchorClamped = Boolean(
    selection &&
      !fullBook &&
      cutoffChapterNumber != null &&
      selection.chapterEnd > cutoffChapterNumber
  );
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setDraft("");
      // 现有 API 仅支持单章锚点；不伪造 selection，不注入假上下文。
      ...
      >
        ...
        {anchorChapterNumber != null ? (
          <p data-testid="analysis-chat-anchor-note">
            上下文锚定第 {anchorChapterNumber} 章
            {anchorClamped ? "（区间超出剧透边界，已收窄）" : ""}
            ；完整章节区间锚点需 25.1-01 后端支持
          </p>
        ) : (
          <p data-testid="analysis-chat-anchor-note">
            章节数据尚未加载，暂时无法发送
