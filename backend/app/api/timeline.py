...
async def get_timeline(
    ...
    return TimelineEnvelope(
        ...
        running_candidate=await build_version_view(
...
async def get_version(
    ...
    active = await build_version_view(
        db,
        novel=novel,
        owner_id=current_user.id,
    ...
    result = next(
        (
            view
            for view in (active, candidate)
            if view and view.version_id == version_id
        ),
        None,
    )
    if result is None:
...
async def rollback(
    ...
    try:
        ...
        raise HTTPException(
            status_code=422, detail="timeline version cannot be activated"
        ) from exc
...
async def edit_event(
    ...
    current = await db.scalar(
        ...
        .where(
            TimelineOverride.owner_id == current_user.id,
            TimelineOverride.novel_id == novel.id,
            TimelineOverride.logical_event_id == logical_event_id,
            TimelineOverride.field_name == data.field_name,
            TimelineOverride.status == "active",
        )
        .order_by(TimelineOverride.id.desc())
        .limit(1)
    )
...
async def set_preference(
    data: TimelinePreferenceRequest,
    novel: Novel = Depends(require_owned_novel),
    db: AsyncSession = Depends(get_db),
):
    novel.reading_progress = {
        **(novel.reading_progress or {}),
        "timeline_full_book": data.full_book,
    }
    await db.commit()
