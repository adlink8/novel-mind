...
class Novel(TimestampMixin, Base):
    ...
    # 基础信息
    title: Mapped[str] = mapped_column(String(200), nullable=False, index=True)      # 小说标题（必填，建立索引用于搜索）
    author: Mapped[str | None] = mapped_column(String(100))                          # 作者（可选，从文件名或内容推断）
    description: Mapped[str | None] = mapped_column(Text)                            # 简介/摘要
    cover_url: Mapped[str | None] = mapped_column(String(500))                       # 封面图片 URL
    genre: Mapped[str | None] = mapped_column(String(50))                            # 类型/流派（玄幻、言情、科幻等）
    ...
    # 文件路径
    source_path: Mapped[str | None] = mapped_column(String(500))                     # 原始 TXT 文件存储路径
...
class Chapter(TimestampMixin, Base):
    ...
    novel_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("novels.id", ondelete="CASCADE"),  # 级联删除
        nullable=False, index=True,                            # 建立索引加速按小说查询章节
    )

    # 章节内容
    chapter_number: Mapped[int] = mapped_column(Integer, nullable=False)             # 章节序号（1, 2, 3...）
    title: Mapped[str] = mapped_column(String(200), default="")                      # 章节标题（如 "第一章 初入江湖"）
    content: Mapped[str] = mapped_column(Text, nullable=False)                       # 章节正文内容（完整文本）
