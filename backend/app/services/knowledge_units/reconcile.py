...
async def reconcile_build(
    db: AsyncSession, *, build_id: int, actual_items: list[dict[str, Any]]
) -> ReconcileReport:
    ...
    expected = tuple(
        f"unit_{unit.canonical_id}_{unit.id}"
        for unit in units
        if unit.status in {"candidate", "active"}
        and unit.lifecycle_status in {"current", "disputed"}
    )
    ...
    wrong_build = tuple(
        sorted(
            ...
            if int(item.get("metadata", {}).get("build_id", -1)) != build.id
            or item.get("metadata", {}).get("manifest_checksum")
    ...
    wrong_owner = tuple(
        sorted(
            str(item["id"])
            for item in actual_items
            if int(item.get("metadata", {}).get("owner_id", -1)) != build.owner_id
            or int(item.get("metadata", {}).get("novel_id", -1)) != build.novel_id
        )
    )
    ...
    deleted = tuple(
        sorted(
            item
            for item in actual
            if item in by_id and by_id[item].lifecycle_status == "deleted"
        )
    )
    deprecated = tuple(
        sorted(
            ...
            if item in by_id
            and (
                by_id[item].lifecycle_status == "deprecated"
                or by_id[item].status == "deprecated"
            )
        )
    )
...
async def read_actual_collection(
    build: NarrativeIndexBuild, store: Any
) -> list[dict[str, Any]]:
    """Read IDs and metadata directly; exported files are never release evidence."""
    if not build.collection_name:
        raise ValueError("build has no collection")
    collection = store.get_named_collection(build.collection_name)
    raw = collection.get(include=["metadatas"])
    return [
        {"id": item_id, "metadata": metadata or {}}
        for item_id, metadata in zip(
            raw.get("ids", []), raw.get("metadatas", []), strict=True
        )
    ]
