...
def validate_resolved_host(host: str, *, resolver=socket.getaddrinfo) -> None:
    ...
    for answer in answers:
        ...
        if address.is_private or address.is_loopback or address.is_link_local or address.is_reserved or address.is_unspecified or address.is_multicast:
            raise ConnectorPolicyError("Tool host resolves to a private or local address")
...
def validate_json_schema(schema: dict) -> None:
    ...
    if not isinstance(required, list) or any(key not in properties for key in required):
        raise ConnectorPolicyError("Tool schema required fields must be declared properties")
...
def validate_json_payload(payload: dict, schema: dict) -> None:
    ...
    if missing:
        raise ConnectorPolicyError(f"Tool request is missing required fields: {missing}")
    if schema.get("additionalProperties") is False:
        unknown = sorted(set(payload) - set(properties))
        if unknown:
            raise ConnectorPolicyError(f"Tool request contains unknown fields: {unknown}")
