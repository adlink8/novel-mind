...
class ReconciliationOutputModel(BaseModel):
    ...
    def as_input(self) -> ReconcileInput:
        return ReconcileInput(
            ...
            causal_edges=[
                CausalProposal(**item.model_dump()) for item in self.causal_edges
            ],
        )
...
def reconciliation_contract_hashes() -> tuple[str, str]:
    ...
    schema_hash = hashlib.sha256(
        json.dumps(
            ReconciliationOutputModel.model_json_schema(),
            sort_keys=True,
        ).encode()
    ).hexdigest()
...
class TimelineReconciler:
    ...
    async def reconcile(
        self, *, run_id: int, events: list[EventCandidate], cache_key: str | None = None
    ) -> ReconcileResult:
        if "quality" not in self.deployment.model_id.lower():
            raise ValueError(
                "reconciliation requires an explicitly qualified quality deployment"
            )
            ...
            try:
                gateway_result = await self.gateway.generate(
                    ...
                    max_input_tokens=max(
                        512, sum(len(e.description) for e in events) * 2
                    ),
    ...
    @staticmethod
    def _materialize(
        events: list[EventCandidate], proposal: ReconcileInput
    ) -> ReconcileResult:
        ...
        for raw_group in proposal.duplicate_groups:
            members = [
                candidate_id for candidate_id in raw_group if candidate_id in by_id
            ]
            if len(members) < 2:
                continue
            root = min(
                members,
                key=lambda key: (
                    by_id[key].narrative_chapter_number,
                    by_id[key].narrative_index,
                    key,
                ),
            )
            merged = sorted(
                set(sum((groups.pop(canonical[item], []) for item in members), []))
            )
            groups[root] = merged
            for item in merged:
                canonical[item] = root
        ...
        constraints = [
            (canonical.get(a, a), canonical.get(b, b))
            for a, b, relation in proposal.story_constraints
            if relation == "before"
        ]
        constraints += [
            (canonical.get(b, b), canonical.get(a, a))
            for a, b, relation in proposal.story_constraints
            if relation == "after"
        ]
        constraints += [
            (
                canonical[exact_sorted[i].candidate_id],
                canonical[exact_sorted[i + 1].candidate_id],
            )
        ...
        for root, members in groups.items():
            source = min(
                (by_id[item] for item in members),
                key=lambda event: (
            ...
            evidence = {
                ref.evidence_id for item in members for ref in by_id[item].evidence
            }
                        ...
                        sorted(
                            {
                                p.mention
                                for item in members
                                for p in by_id[item].participants
                            }
                        )
                    ),
                    tuple(sorted(evidence)),
                    (source.narrative_chapter_number, source.narrative_index),
                    ranks.get(root),
                )
            )
        ...
        for edge in proposal.causal_edges:
            source, target = (
                canonical.get(edge.source_id),
                canonical.get(edge.target_id),
            )
            if not source or not target or source == target:
                continue
            refs = set(edge.evidence_ids)
            if not (
                refs & evidence_by_root[source] and refs & evidence_by_root[target]
            ):
                continue
            edges.append(
                ReconciledEdge(
                    source, target, edge.edge_type, tuple(sorted(refs)), edge.confidence
                )
            )
