    ...
    event_rows = [
        {
            ...
            "exact_time": (
                row.exact_time.isoformat() if row.exact_time is not None else None
            ),
            "relative_anchor_event_id": row.relative_anchor_event_id,
            "relative_relation": row.relative_relation,
            "fuzzy_start": (
                row.fuzzy_start.isoformat() if row.fuzzy_start is not None else None
            ),
            "fuzzy_end": (
                row.fuzzy_end.isoformat() if row.fuzzy_end is not None else None
            ),
