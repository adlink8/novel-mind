...
class ExactCacheKey:
    ...
    def for_package(cls, package: EvidencePackage, **identity: str) -> "ExactCacheKey":
        return cls(
            identity["stage"], package.source_snapshot_hash, package.hierarchy_build_id,
            package.hierarchy_checksum, package.unit_id, package.package_hash,
            identity["prompt_hash"], identity["schema_hash"], identity["model_provider"],
            identity["model_id"], identity["model_revision"], identity["decoding_hash"],
...
class TimelineChapterExtractor:
    def __init__(self, gateway: TimelineModelGateway, store: InMemoryExtractionStore, *,
                 deployment: ModelDeployment, budget: BudgetGate, prompt: str,
                 prompt_hash: str, schema_hash: str, decoding_hash: str, config_hash: str) -> None:
    ...
    async def extract(self, *, run_id: int, version_id: int,
                      package: EvidencePackage) -> ExtractionResult:
        key = ExactCacheKey.for_package(
            package, stage="chapter_extract", prompt_hash=self.prompt_hash,
            schema_hash=self.schema_hash, model_provider=self.deployment.provider,
            model_id=self.deployment.model_id, model_revision=self.deployment.revision,
            decoding_hash=self.decoding_hash, config_hash=self.config_hash,
        )
        ...
        if cached is not None:
            ...
            self.store.audits.append(ExtractionAudit(
                self.store.next_attempt_id(), run_id, stage_key, "call-skipped", key.digest,
                cached.artifact_checksum, cached.source_attempt_id,
            ))
            return ExtractionResult(events, cached.artifact_checksum,
                                    cached.source_attempt_id, True)
        ...
        try:
            result = await self.gateway.generate(
                deployment=self.deployment, schema=TimelineExtraction, messages=messages,
                budget=self.budget, run_id=run_id, stage_key=stage_key,
            ...
            for attempt in exc.attempts:
                self.store.audits.append(ExtractionAudit(
                    self.store.next_attempt_id(), run_id, stage_key, attempt.status,
                    key.digest, None, gateway_attempt=attempt,
                ))
            raise
        ...
        for attempt in result.attempts:
            ...
            self.store.audits.append(ExtractionAudit(
                attempt_id, run_id, stage_key, attempt.status, key.digest, None,
        ...
        self.store.audits[-1] = ExtractionAudit(
            source_attempt_id, run_id, stage_key, "succeeded", key.digest,
            artifact_checksum, gateway_attempt=result.attempts[-1],
        )
        self.store.cache[key.digest] = CacheEntry(result.output, artifact_checksum, source_attempt_id)
    ...
    def _publish(self, version_id: int, package: EvidencePackage,
                 extraction: TimelineExtraction) -> list[PublishedCandidate]:
        validate_extraction(package, extraction)
        events = [PublishedCandidate(event, package.owner_id, package.novel_id, package.chapter_id)
    ...
    def _package_payload(package: EvidencePackage) -> str:
        return json.dumps({
            "scope": {"owner_id": package.owner_id, "novel_id": package.novel_id,
                      "chapter_id": package.chapter_id, "unit_id": package.unit_id},
