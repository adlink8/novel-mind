...
def stable_stage_key(
    stage: str, *, chapter_id: int | None = None, attempt: int | None = None
) -> str:
...
class TimelineJobStore(Protocol):
    ...
    async def acquire_lease(
        self, run_id: int, lease_id: str, now: datetime, expires: datetime
    ) -> bool: ...
    async def complete_stage(
        self, run_id: int, stage_key: str, checksum: str
    ) -> None: ...
    async def set_status(
        self, run_id: int, status: str, *, cancel_requested: bool = False
    ) -> None: ...
    ...
    async def acquire_lease(
        ...
        async with self._lock:
            ...
            if row.lease_id and row.lease_expires_at and row.lease_expires_at > now:
                return False
            row.lease_id, row.lease_expires_at, row.status = (
                lease_id,
                expires,
                "running",
            )
            ...
            stages = (
                await session.execute(
                    select(AnalysisChapterStage).where(
                        AnalysisChapterStage.run_id == run_id,
                        AnalysisChapterStage.status == "completed",
                    )
                )
            ).scalars()
            return self._record(
                row, {s.stage_key: s.artifact_checksum or "" for s in stages}
            )
...
class TimelineJobCoordinator:
    ...
    async def acquire_lease(
        self, run_id: int, *, now: datetime | None = None
    ) -> str | None:
        now = now or datetime.now(UTC)
        lease_id = uuid.uuid4().hex
        if await self.store.acquire_lease(
            run_id, lease_id, now, now + timedelta(seconds=self.lease_seconds)
        ):
            return lease_id
        return None
