"""PostgreSQL local-rebuild executor tests (carry + dirty-only Phase 14 stages)."""
...
async def _seed(session: AsyncSession, *, edit_chapter: int | None = None) -> dict:
    user = User(
        username="local-rebuild-owner",
        email="local-rebuild@example.com",
        hashed_password="x",
    )
    session.add(user)
    await session.flush()
    novel = Novel(owner_id=user.id, title="Local Rebuild", status="ready")
    session.add(novel)
    await session.flush()
    ...
    parent_node_snapshot = {
        n.node_key: n.content_checksum
        for n in (
            await session.scalars(
                select(NarrativeMemoryNode).where(
                    NarrativeMemoryNode.version_id == parent.id
                )
            )
        ).all()
    }
    ...
    return {
        ...
        "parent_node_snapshot": parent_node_snapshot,
    }


@pytest.fixture
async def rebuild_env(empty_postgres: str, pg_async_url: str):
...
@pytest.mark.asyncio
async def test_no_change_zero_dirty_stages(rebuild_env) -> None:
    assert executor_has_provider_capability() is False
    async with rebuild_env() as session:
    ...
    async with rebuild_env() as session:
        ...
        await session.commit()
        assert carry.carried_node_keys
        assert mask.dirty_stage_keys == () or all(
            k.startswith("arc_volume_plan") is False for k in mask.dirty_stage_keys
        )
        # Full carry → no dirty semantic stages
        dirty_semantic = [
            i
            for i in plan.items
...
@pytest.mark.asyncio
async def test_edit_materializes_dirty_only_stages(rebuild_env) -> None:
    async with rebuild_env() as session:
    ...
    async with rebuild_env() as session:
        ...
        carried_stage_keys = {
            i.stage_key
            for i in plan.items
            if i.decision == RebuildDecision.CARRIED and i.stage_key
        }
        dirty_stage_keys = {
            i.stage_key
            for i in plan.items
            if i.decision == RebuildDecision.DIRTY and i.stage_key
        }
        assert dirty_stage_keys
        ...
        for key in carried_stage_keys:
            assert key not in stage_keys
        for key in dirty_stage_keys:
            if key:  # some dirty assets may share stage materialization rules
                assert key in stage_keys or key in mask.dirty_stage_keys

        # Full conservative dirty closure may leave nothing to carry; stages must still
        # be dirty-only and parent authority byte-stable.
        assert set(carry.skipped_dirty_keys) >= {
            i.asset_key
            for i in plan.items
            if i.decision == RebuildDecision.DIRTY
            and i.asset_kind.value in {"chapter_state", "story_arc", "global_story"}
        ...
        with pytest.raises(RebuildExecutorError):
            assert_stage_allowed(mask, "chapter_state:not-in-plan")
        ...
        assert parent_nodes == ctx["parent_node_snapshot"]

        run = await session.get(NarrativeMemoryBuildRun, run_id)
        assert run is not None
        assert run.progress.get("rebuild_plan_checksum") == row.plan_checksum
        assert "carried_asset_keys" in run.progress


@pytest.mark.asyncio
async def test_plan_checksum_mismatch_blocks_execute(rebuild_env) -> None:
    async with rebuild_env() as session:
    ...
    async with rebuild_env() as session:
        ...
        with pytest.raises(RebuildExecutorError):
            await materialize_carry_and_dirty_stages(
                ...
                plan_id=row.id,
                run_policy=_run_policy(),
                expected_plan_checksum="e" * 64,
            )


@pytest.mark.asyncio
async def test_mask_excludes_carried_from_stage_specs(rebuild_env) -> None:
    async with rebuild_env() as session:
        ctx = await _seed(session, edit_chapter=2)
    ...
    async with rebuild_env() as session:
        ...
        db_plan = await session.get(NarrativeMemoryRebuildPlan, row.id)
        items = list(
            (
                await session.scalars(
        ...
        mask = build_dirty_stage_mask(db_plan, items)
        for key in mask.carried_asset_keys:
            item = next(i for i in items if i.asset_key == key)
            if item.stage_key:
                assert item.stage_key not in mask.dirty_stage_keys
        assert all(s["stage_key"] in mask.dirty_stage_keys for s in mask.stage_specs)
        # Carried boundary must not be forced into dirty keys
        for item in items:
            if (
