...
async def test_autosave_appends_immutable_lineage(api_client):
    ...
    first = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "A", 1
    )
    ...
    second = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "B", 2
    )
    ...
    third_detail = await client.get(
        f"{url}/revisions/{body['items'][0]['id']}", headers=headers
    )
...
async def test_autosave_noop_is_idempotent(api_client):
    ...
    dup = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "A", 2
    )
...
async def test_stale_base_conflict_returns_409_with_latest_revision(api_client):
    ...
    stale = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "B", 1
    )
...
async def test_concurrent_autosaves_never_last_write_win(api_client):
    ...
    assert (
        loser.json()["detail"]["current_revision"]["content"]
        == winner.json()["revision"]["content"]
    )
...
async def test_diff_between_revisions_is_deterministic(api_client):
    ...
    ops = [
        (line["op"], line["text"]) for hunk in body["hunks"] for line in hunk["lines"]
    ]
...
async def test_rollback_requires_current_base_revision(api_client):
    ...
    stale = await _rollback(
        client, headers, novel_id, project["id"], chapter["id"], root_id, 1
    )
    assert stale.status_code == 409, stale.text
    detail = stale.json()["detail"]
    assert detail["code"] == "revision_conflict"
...
async def test_rollback_then_continue_editing(api_client):
    ...
    cont = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "C", 4
    )
    assert cont.status_code == 200, cont.text
    assert cont.json()["status"] == "saved"
    assert cont.json()["chapter"]["revision"] == 5
    assert (
        cont.json()["revision"]["parent_revision_id"]
        == rollback.json()["revision"]["id"]
    )
...
async def test_foreign_owner_cannot_touch_revision_surface(api_client):
    ...
    await _autosave(
        client, headers_a, a["novel_id"], project_a["id"], chapter_a["id"], "A", 1
    )
    ...
    assert (
        await client.get(f"{base_b}/revisions", headers=headers_b)
    ).status_code == 404
    assert (
        await client.post(
            f"{base_b}/autosave",
            json={"content": "x", "base_revision": 1},
            headers=headers_b,
        )
    ).status_code == 404
    assert (
        await client.post(
            f"{base_b}/rollback",
            json={"target_revision_id": 1, "base_revision": 1},
            headers=headers_b,
        )
    ).status_code == 404
    assert (
...
async def test_archived_project_blocks_revision_writes(api_client):
    ...
    autosave = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "x", 1
    )
    assert (
        autosave.status_code == 409
        and "project_archived" in autosave.json()["detail"]["code"]
    )
    rollback = await _rollback(
        client, headers, novel_id, project["id"], chapter["id"], 1, 1
    )
    assert (
        rollback.status_code == 409
        and "project_archived" in rollback.json()["detail"]["code"]
    )
...
async def test_unauthenticated_revision_routes_reject(api_client):
    ...
    assert (
        await client.post(f"{base}/autosave", json={"content": "x", "base_revision": 1})
    ).status_code == 401
    assert (await client.get(f"{base}/revisions")).status_code == 401
    assert (await client.get(f"{base}/revisions/1")).status_code == 401
    assert (
        await client.get(
            f"{base}/diff", params={"base_revision_id": 1, "target_revision_id": 1}
        )
    ).status_code == 401
    assert (
        await client.post(
            f"{base}/rollback", json={"target_revision_id": 1, "base_revision": 1}
        )
    ).status_code == 401
...
async def test_autosave_is_durable_after_crash_before_ack(api_client):
    ...
    saved = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "durable", 1
    )
    assert saved.status_code == 200 and saved.json()["status"] == "saved"
    revision_id = saved.json()["revision"]["id"]
...
async def test_revision_migration_replays(pg_sync_url, require_postgres):
    ...
    with engine.connect() as conn:
        ...
        uniques = {
            c["name"] for c in insp.get_unique_constraints("derivative_revisions")
        }
...
async def test_agent_proposal_apply_appends_distinct_kind(api_client):
    ...
    cont = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "user continues", 2
    )
    assert cont.status_code == 200, cont.text
    assert cont.json()["status"] == "saved"
...
async def test_agent_proposal_apply_stale_base_blocks(api_client):
    ...
    saved = await _autosave(
        client, headers, novel_id, project["id"], chapter["id"], "user head", 1
    )
    assert saved.status_code == 200 and saved.json()["status"] == "saved"
