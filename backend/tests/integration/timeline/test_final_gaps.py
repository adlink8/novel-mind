...
async def _seed_run(session):
    ...
    for index, chapter in enumerate(chapters):
        session.add(
            ChunkHierarchyNode(
                ...
                source_end=index * 100 + len(chapter.content),
    ...
    run = AnalysisRun(
        owner_id=owner.id, novel_id=novel.id, status="pending", active_key=None
    )
    session.add(run)
...
@pytest.mark.parametrize(
    "boundary", ["evidence", "extraction", "persist", "reconcile", "promotion"]
)
async def test_running_worker_observes_cancel_between_every_production_stage(
    ...
    if boundary == "evidence":
        ...
        async def cancel_after_first_extraction(schema_name):
            if (
                schema_name == "TimelineExtraction"
                and transport.calls.count(schema_name) == 1
            ):
                await _request_cancel(sessions, run_id)
        ...
        monkeypatch.setattr(
            worker_module, "_reconcile_and_persist", cancel_before_promotion
        )
    ...
    assert (
        await db_session.scalar(
            select(TimelineActivePointer.id).where(
...
async def test_reconciliation_cache_misses_after_lineage_hash_change(
    ...
    changed_run = AnalysisRun(
        owner_id=baseline_run.owner_id,
        novel_id=novel.id,
        status="pending",
    )
    ...
    async def mutate_lineage_after_restart(*args, **kwargs):
        ...
        async with runtime.sessions.begin() as session:
            persisted = await session.get(
                type(version), version.id, with_for_update=True
            )
...
async def test_real_api_serializes_persisted_source_start_and_orders_by_it(
    ...
    first_evidence = await db_session.scalar(
    ...
    response = await auth_client.get(
        f"/api/timeline/{novel_id}?ordering=narrative&full_book=true"
    )
...
async def test_qualification_compares_default_and_full_queries_and_rechecks_db_authority(
    ...
    observed = await load_persisted_authority(
        runtime.sessions, report["artifact"]["authority"]
    )
