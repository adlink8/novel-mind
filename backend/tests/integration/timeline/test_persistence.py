...
def test_timeline_metadata_contains_authority_tables():
    ...
    expected = {
        "analysis_runs",
        "analysis_versions",
        "analysis_chapter_stages",
        "model_call_attempts",
        "analysis_budget_ledgers",
        "analysis_budget_reservations",
...
def test_owner_scope_and_override_relink_are_persisted():
    ...
    assert {"supersedes_id", "status", "needs_relink"} <= set(
        inspect(TimelineOverride).columns.keys()
    )
