...
@pytest.mark.asyncio
async def test_postgres_stale_cas_failed_candidate_and_byte_identical_rollback(
    ...
    async with factory() as setup:
        user = User(
            username=f"timeline-{suffix}",
            email=f"timeline-{suffix}@example.test",
            hashed_password="x",
        )
        setup.add(user)
        await setup.flush()
        ...
        await setup.flush()
        v1 = await _candidate(
            setup, user.id, novel.id, f"v1-{suffix}", "old", "ev-stable", chapter.id
        )
        v2 = await _candidate(
            setup, user.id, novel.id, f"v2-{suffix}", "new", "ev-stable", chapter.id
        )
        bad = await _candidate(
            setup, user.id, novel.id, f"bad-{suffix}", "bad", "ev-bad", chapter.id
        )
        ...
        await setup.commit()
        owner_id, novel_id, v1_id, v2_id, bad_id = (
            user.id,
            novel.id,
            v1.id,
            v2.id,
    ...
    async with factory() as session:
        ...
        journal = (
            await session.scalars(
                select(TimelinePointerJournal)
                .where(TimelinePointerJournal.owner_id == owner_id)
        ...
        override = (
            await session.scalars(
                select(TimelineOverride).where(TimelineOverride.owner_id == owner_id)
            )
        ).one()
