...
def _result(kind: AssetKind) -> AssetEligibility:
    return AssetEligibility(
        kind=kind,
        requirement=(AssetRequirement.REQUIRED if kind == AssetKind.HIERARCHY else AssetRequirement.OPTIONAL),
        owner_id=1,
        novel_id=2,
...
def test_inventory_is_strict_and_canonical() -> None:
    inventory = AssetInventory(
        ...
        rebuild_ranges=(RebuildRange(start_chapter=4, end_chapter=5), RebuildRange(start_chapter=1, end_chapter=1)),
    )
    assert inventory.reason_codes == (ReasonCode.MANIFEST_MISMATCH, ReasonCode.STALE_ASSET)
...
def test_report_requires_all_unique_assets_and_matching_scope() -> None:
    ...
    assert [item.kind.value for item in report.assets] == sorted(kind.value for kind in AssetKind)
    ...
    forged = list(assets)
    hierarchy_index = next(index for index, item in enumerate(forged) if item.kind == AssetKind.HIERARCHY)
