...
def _ref(
    chapter_id, chapter_number, start, end, *, ss_id="ss-1", ss_hash=HEX64
) -> VisualEvidenceRef:
...
def _claim(
    *, evidence_refs, authority=VisualAuthority.CANON_FACT
) -> VisualClaimContract:
...
def test_compute_source_snapshot_hash_binds_scope_and_content():
    ...
    assert h1 == h2
    other_owner = compute_source_snapshot_hash(
        owner_id=2, novel_id=1, chapters=chapters
    )
    assert other_owner != h1
    mutated = [ChapterRecord(chapter_id=1, chapter_number=1, content=TEXT + "补充")]
...
async def _seed(db_session: AsyncSession):
    ...
    await db_session.flush()
    chapter = Chapter(novel_id=novel.id, chapter_number=1, title="第一章", content=TEXT)
...
async def test_verify_novel_scope(db_session):
    ...
    assert (
        await service.verify_novel_scope(owner_id=owner.id, novel_id=novel.id)
    ).id == novel.id
...
async def test_materialize_resolves_canon_claim(db_session):
    owner, novel, chapter = await _seed(db_session)
    service = VisualBibleEvidenceService(db_session)
    snapshot_hash, _ = await service.load_source_snapshot(
        owner_id=owner.id, novel_id=novel.id
    )
