...
def _event(
    ...
    *,
    ...
    story_time = (
        StoryTime(precision="exact", expression="明确时刻", exact_time=exact)
        if exact
        else StoryTime(precision="unknown")
    )
    return EventCandidate(
        ...
        participants=[Participant(mention=mention)],
...
async def test_reconcile_uses_quality_deployment_and_reserves_before_transport():
    ...
    deployment = ModelDeployment(
        "openai", "quality", "r1", True, Decimal("1"), Decimal("2")
    )
    reconciler = TimelineReconciler(
        transport=transport, deployment=deployment, budget=_budget()
    )
    result = await reconciler.reconcile(
        run_id=7,
        events=[
            _event(
                "a", 1, 0, title="启程", exact=datetime(2020, 1, 1, tzinfo=timezone.utc)
            ),
            _event("a-repeat", 2, 0, title="回忆启程"),
            _event(
                "b", 3, 0, title="抵达", exact=datetime(2020, 1, 2, tzinfo=timezone.utc)
            ),
        ],
    )
...
async def test_cycles_and_contradictions_are_retained_without_fabricated_order():
    ...
    result = await reconciler.reconcile(
        run_id=9, events=[_event("a", 1, 0, title="甲"), _event("b", 1, 1, title="乙")]
    )
