...
async def _accepted_source(db, *, risk_flags=None):
    ...
    snapshot = await source_snapshot_service.create_snapshot(
        db, owner_id=user.id, novel_id=novel.id, domain_profile="fiction"
    )
    return snapshot
...
async def test_materializes_grounded_unit_and_links(db_session):
    ...
    report = await narrative_unit_materializer.materialize_snapshot(
...
async def test_materialization_is_idempotent(db_session):
    snapshot = await _accepted_source(db_session)
    first = await narrative_unit_materializer.materialize_snapshot(
        db_session, snapshot_id=snapshot.id
    )
    second = await narrative_unit_materializer.materialize_snapshot(
        db_session, snapshot_id=snapshot.id
    )
...
async def test_dry_run_does_not_write(db_session):
    snapshot = await _accepted_source(db_session)
    report = await narrative_unit_materializer.materialize_snapshot(
        db_session, snapshot_id=snapshot.id, write=False
    )
    assert report.created == 1
