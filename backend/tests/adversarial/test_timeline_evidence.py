...
def package():
    ...
    return EvidencePackage.create(
        owner_id=1, novel_id=2, chapter_id=9, unit_id="scene-9",
        source_snapshot_hash="a" * 64, hierarchy_build_id="b1", hierarchy_checksum="b" * 64,
...
def candidate(**evidence_overrides):
    p = package()
    evidence = {
        "chapter_id": 9, "evidence_id": "safe-1", "source_start": 0,
        "source_end": p.units[0].source_end, "content_hash": p.units[0].content_hash,
    }
    evidence.update(evidence_overrides)
    return TimelineExtraction.model_validate({"events": [{
        "candidate_id": "x", "title": "Coronation", "description": "A coronation occurs.",
        "event_type": "plot", "narrative_chapter_number": 9, "narrative_index": 0,
        "participants": [], "story_time": {"precision": "unknown", "expression": None},
